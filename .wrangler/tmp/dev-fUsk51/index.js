...
// src/lib/d1.js
async function insertarDesdeJSON(db, tabla, data) {
...
__name(insertarDesdeJSON, "insertarDesdeJSON");

// src/routes/importar.js
var app = new Hono2();
app.post("/", async (c) => {
  ...
  return sign2(
    {
      sub: String(user.id),
      role: user.role,
      exp: Math.floor(Date.now() / 1e3) + 60 * 2
      // 2 min
    },
    c.env.JWT_SECRET,
    "HS256"
...
async function createRefreshToken(userId, kv, ttlDays = 14) {
  const raw2 = randomToken();
  const hash = await sha256(raw2);
...
app2.post("/auth/refresh", async (c) => {
...
app2.post("/auth/register", async (c) => {
...
app2.route("/api/importar", importar_default);
app2.get("/debug/env", (c) => {
  return c.json({
    message: "env ok",
    env: c.env
  });
...
// .wrangler/tmp/bundle-DOgbOA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DOgbOA/middleware-loader.entry.ts
